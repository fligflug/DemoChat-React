describe('DemoChat', function() {
    ...
    it('chat_connection_ok', function(){
        ...
        cy.get('#standard-basic').type('Flig flug');
    ...
    it('chat_connection_nickname_too_long', function(){
        cy.visit('http://0.0.0.0:8080');
        cy.get('#standard-basic').type('Fliiiiiiiig Fluuuuuuug');
        cy.get('button').should('have.class', 'Mui-disabled');
        cy.get('#standard-basic-helper-text').contains('Nickname must be less than 20 characters long.');
    });
    ...
    it('chat_send_new_message_ok', function(){
        ...
        cy.get('.message').contains('Hello there!');
    });
    ...
    it('chat_send_new_message_empty', function(){
        localStorage.setItem('nickname', 'Flig Flug');
        cy.visit('http://0.0.0.0:8080/chat');
        cy.get('button').first().should('have.class', 'Mui-disabled');
    });
